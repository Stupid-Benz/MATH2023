...
\[
  \proj_{\mathbf{a}} \mathbf{b} = \left(\frac{\mathbf{a} \cdot \mathbf{b}}{\|\mathbf{a}\|}\right) \frac{\mathbf{a}}{\|\mathbf{a}\|} = \frac{\mathbf{a} \cdot \mathbf{b}}{\|\mathbf{a}\|^2} \mathbf{a}
\]
...
\begin{solution}
  ...
  \begin{equation}
    \text{Distance} = \| \proj_{\mathbf{n}} \overrightarrow{P_1P_2} \| = \frac{|\overrightarrow{P_1P_2} \cdot \mathbf{n}|}{\|\mathbf{n}\|} = \frac{|(\mathbf{r_2} - \mathbf{r_1}) \cdot (\mathbf{v_1} \times \mathbf{v_2})|}{\|\mathbf{v_1} \times \mathbf{v_2}\|}.
  \end{equation}
\end{solution}
...
\begin{proof}
  ...
  \[
    \begin{split}
      \frac{dz}{dt}\bigg|_{t = t_0} & = \lim_{t \to t_0} \frac{f(x(t), y(t)) - f(x_0, y_0)}{t - t_0}                                                                                                        \\
                                    & = \lim_{t \to t_0} \left[ (f_x(x(t_0), y(t_0)) + \epsilon_1) \frac{x(t) - x(t_0)}{t - t_0} + (f_y(x(t_0), y(t_0)) + \epsilon_2) \frac{y(t) - y(t_0)}{t - t_0} \right] \\
                                    & = f_x(x_0, y_0) \frac{dx}{dt}\bigg|_{t = t_0} + f_y(x_0, y_0) \frac{dy}{dt}\bigg|_{t = t_0}.
    \end{split}
  \]
\end{proof}
...
\begin{table}[!ht]
  \centering
  \begin{tabular}{c c | c}
    $\left(f_{xx} f_{yy} - f^2_{xy}\right)\bigg|_{(x_0, y_0)}$ & $f_{xx}(x_0, y_0)$ & $(x_0, y_0)$ is a \\
    \hline
    $+$                                                        & $+$                & local minimum     \\
    $+$                                                        & $-$                & local maximum     \\
    $-$                                                        & any                & saddle point      \\
...
\begin{equation}\label{eq:line_integral_scalar}
...
\begin{equation}
  \int_C \mathbf{F} \cdot \mathbf{T} \, ds = \int_C \mathbf{F} \cdot d\mathbf{r} = \int_a^b P dx + Q dy + R dz,
\end{equation}
...
\begin{itemize}
  \item The operator $d$ applied to the 0-form $f$ is the differential $df$ of $f$;
  \item If $\omega_1$ and $\omega_2$ are two $k$-forms, then $d(a\omega_1 + b\omega_2) = a \, d\omega_1 + b \, d\omega_2$ for any scalars $a$ and $b$;
  \item If $\omega_1$ is a $k$-form and $\omega_2$ is an $l$-form, then $d(\omega_1 \wedge \omega_2) = d\omega_1 \wedge \omega_2 + {(-1)}^k \omega_1 \wedge d\omega_2$;
  \item If $\omega$ is a $k$-form, then $d(d\omega) = 0$.
\end{itemize}
Also, we have some properties of exterior products:
\begin{itemize}
  \item Alternating property: $\omega \wedge \omega = 0$ and $\omega_1 \wedge \omega_2 = {(-1)}^{kl} \omega_2 \wedge \omega_1$ where $\omega_1$ is a $k$-form and $\omega_2$ is an $l$-form;
  \item Associative property: $(\omega_1 \wedge \omega_2) \wedge \omega_3 = \omega_1 \wedge (\omega_2 \wedge \omega_3)$;
  \item Distributive property: $\omega_1 \wedge (\omega_2 + \omega_3) = \omega_1 \wedge \omega_2 + \omega_1 \wedge \omega_3$.
\end{itemize}
...
\begin{example}
  Let $\mathbf{F} = \langle 3 + 2xy, x^2 - 3y^2 \rangle$. Show that $\mathbf{F}$ is a conservative vector field and find a potential function $\phi$ such that $\mathbf{F} = \nabla \phi$.
\end{example}
\begin{solution}
  Note that $\mathbf{F}$ is defined on the entire $\mathbb{R}^2$ and the components of $\mathbf{F}$ have continuous first partial derivatives. Also, we have:
  \[
    \frac{\partial}{\partial y} (3 + 2xy) = 2x = \frac{\partial}{\partial x} (x^2 - 3y^2).
  \]
  Thus, $\mathbf{F}$ is a conservative vector field. We have two ways to find the potential function $\phi$:
  \begin{enumerate}
    \item To solve the following system of equations:
          \[
            \frac{\partial \phi}{\partial x} = 3 + 2xy, \quad \frac{\partial \phi}{\partial y} = x^2 - 3y^2.
          \]
          Integrating the first equation with respect to $x$, we get:
          \[
            \phi(x,y) = 3x + x^2 y + g(y),
          \]
          where $g(y)$ is an arbitrary function of $y$. Next, we differentiate $\phi(x,y)$ with respect to $y$ and set it equal to the second equation:
          \[
            \frac{\partial \phi}{\partial y} = x^2 + g'(y) = x^2 - 3y^2.
          \]
          Thus, we have $g'(y) = -3y^2$, which implies that $g(y) = -y^3 + C$ for some constant $C$. Therefore, the potential function $\phi$ is given by:
          \[
            \phi(x,y) = 3x + x^2 y - y^3 + C.
          \]
    \item As $\mathbf{F}$ is a conservative vector field, we have $\phi$ and $\mu$ such that $\nabla \phi = \nabla \mu$, i.e., there is a function $\psi = \phi - \mu$ such that $\nabla \psi = \mathbf{0}$. Thus, $\psi$ is a constant function. It suffices to show that $\psi(P) = \psi(Q)$ for any two points $P$ and $Q$ in $\mathbb{R}^2$. Let $C$ be any smooth curve connecting $P$ and $Q$. Then we have:
          \[
            0 = \int_C \nabla \psi = \psi(Q) - \psi(P).
          \]
          Then we want to find the value of $\phi$ at any point $(x,y)$ starting from the origin $(0,0)$. Let $C$ be the piecewise smooth curve consisting of the line segment from $(0,0)$ to $(x,0)$ and the line segment from $(x,0)$ to $(x,y)$. Then we have:
          \begin{align*}
            \phi(x,y) - \phi(0,0) & = \int_C \mathbf{F} \cdot d\mathbf{r} = \int_{C_1} \mathbf{F} \cdot d\mathbf{r} + \int_{C_2} \mathbf{F} \cdot d\mathbf{r} \\
                                  & = \int_0^x (3 + 0) \, dx + \int_0^y (3 + 2x(0)) \, dy                                                                     \\
                                  & = 3x + (x^2 y - y^3).
          \end{align*}
          Thus, the potential function $\phi$ is given by:
          \[
            \phi(x,y) = 3x + x^2 y - y^3 + \phi(0,0).
          \]
  \end{enumerate}
\end{solution}
...
\begin{theorem}[Green's Theorem]
  ...
  \begin{equation}
    \oint_C \mathbf{F} \cdot \mathbf{T} \, ds = \oint_C P \, dx + Q \, dy = \iint_D \left( \frac{\partial Q}{\partial x} - \frac{\partial P}{\partial y} \right) \, dA = \iint_D \Curl \, \mathbf{F} \cdot \mathbf{k} \, dA.
  \end{equation}
\end{theorem}
The operator $\Curl$ will be defined later. Here, we provide a proof using the generalised Stokes' Theorem.

Consider the differential 1-form $\omega = P \, dx + Q \, dy$ on the region $D$, and its exterior derivative is given by:
\begin{align*}
  d\omega & = dP \wedge dx + P \, d(dx) + dQ \wedge dy + Q \, d(dy)                                                                                                                                                   \\
...
Thus, by the generalised Stokes' Theorem, we have:
\[
  \oint_C P \, dx + Q \, dy = \iint_D \left( Q_x - P_y \right) \, dx \wedge dy.
\]
...
We may introduce the concept of curl. Let $\mathbf{F}(x,y,z) = \langle P(x,y,z), Q(x,y,z), R(x,y,z) \rangle$ be a vector field in three dimensions. The curl of $\mathbf{F}$ is defined as:
\begin{equation}
...
\end{equation}
For easier memorisation, we can express the curl of $\mathbf{F}$ using the following determinant:
\begin{equation}
...
\begin{proposition}
  If $f$ is a scalar function with continuous second-order partial derivatives, then we have:
  \begin{equation}
    \Curl \, (\nabla f) = \mathbf{0}.
  \end{equation}
  Or equivalently, if a vector field $\mathbf{F}$ is conservative, then its curl is zero.
\end{proposition}
\begin{proof}
  We have:
  \[
  ...
  \]
  Thus, the curl of $\mathbf{F}$ is given by:
  \begin{align*}
    \Curl \, \mathbf{F} & = \left\langle \frac{\partial^2 f}{\partial y \partial z} - \frac{\partial^2 f}{\partial z \partial y}, \frac{\partial^2 f}{\partial z \partial x} - \frac{\partial^2 f}{\partial x \partial z}, \frac{\partial^2 f}{\partial x \partial y} - \frac{\partial^2 f}{\partial y \partial x} \right\rangle \\
                        & = \langle 0, 0, 0 \rangle = \mathbf{0}.
  \end{align*}
\end{proof}

The converse of the above proposition generally does not hold. However, if the domain of the vector field is simply connected, then the converse holds.

\begin{definition}[Simply Connected]
  A region $D$ in $\mathbb{R}^2$ is said to be simply connected if any closed curve in $D$ can be continuously contracted to a point without leaving $D$. Similarly, a region $E$ in $\mathbb{R}^3$ is said to be simply connected if any closed surface in $E$ can be continuously contracted to a point without leaving $E$.
\end{definition}

\begin{proposition}
  If $\mathbf{F}$ is a vector field defined on a simply connected region in $\R^3$ whose components have continuous first partial derivatives, and if $\Curl \, \mathbf{F} = \mathbf{0}$, then $\mathbf{F}$ is a conservative vector field.
\end{proposition}
The proof requires the use of Stokes' Theorem.

\begin{example}
  Show that $\mathbf{F}(x, y, z) = \langle y^2 z^3, 2xyz^3, 3xy^2 z^2 \rangle$ is conservative. Hence, find a function $f$ such that $\mathbf{F} = \nabla f$.
\end{example}
\begin{solution}
  ...
  \begin{align*}
    \Curl \, \mathbf{F} & = \begin{vmatrix}
                            ...
                            \end{vmatrix} \\
                        & = \langle 6xyz^2 - 6xyz^2, 3y^2 z^2 - 3y^2 z^2, 2yz^3 - 2yz^3 \rangle                        \\
                        & = \langle 0, 0, 0 \rangle = \mathbf{0}.
  \end{align*}
  Since $\mathbf{F}$ is defined on the entire $\R^3$ and the components of $\mathbf{F}$ have continuous first partial derivatives, by the above proposition, $\mathbf{F}$ is a conservative vector field.

  Next, we find the potential function $f$ such that $\mathbf{F} = \nabla f$. We have the following system of equations:
  \[
    f_x = y^2 z^3, \quad f_y = 2xyz^3, \quad f_z = 3xy^2 z^2.
  \]
  Integrating the first equation with respect to $x$, we get:
  \[
    f(x,y,z) = xy^2 z^3 + g(y,z),
  \]
  where $g(y,z)$ is an arbitrary function of $y$ and $z$. Next, we differentiate $f(x,y,z)$ with respect to $y$ and set it equal to the second equation:
  \[
    f_y = 2xyz^3 + \frac{\partial g}{\partial y} = 2xyz^3.
  \]
  Thus, we have $\frac{\partial g}{\partial y} = 0$, which implies that $g(y,z) = h(z)$ for some function $h(z)$. Then we differentiate $f(x,y,z)$ with respect to $z$ and set it equal to the third equation:
  \[
    f_z = 3xy^2 z^2 + h'(z) = 3xy^2 z^2.
  \]
  Thus, we have $h'(z) = 0$, which implies that $h(z) = C$ for some constant $C$. Therefore, the potential function $f$ is given by:
  \[
    f(x,y,z) = xy^2 z^3 + C.
  \]
\end{solution}
...
Let $\mathbf{F}(x,y,z) = \langle P(x,y,z), Q(x,y,z), R(x,y,z) \rangle$ be a vector field in three dimensions. The divergence of $\mathbf{F}$ is defined as:
\begin{equation}
  \Div \, \mathbf{F} = \frac{\partial P}{\partial x} + \frac{\partial Q}{\partial y} + \frac{\partial R}{\partial z}.
\end{equation}
The divergence can be denoted using the following notation:
\begin{equation}
  \Div \, \mathbf{F} = \nabla \cdot \mathbf{F} = \frac{\partial P}{\partial x} + \frac{\partial Q}{\partial y} + \frac{\partial R}{\partial z}.
\end{equation}

\begin{proposition}
  If $\mathbf{F}$ is a vector field on $\R^3$ and $P$, $Q$ and $R$ have continuous second-order partial derivatives, then we have:
  \begin{equation}
    \Div \, \Curl \, \mathbf{F} = \nabla \cdot (\nabla \times \mathbf{F}) = 0.
  \end{equation}
\end{proposition}
\begin{proof}
  Using the definition of curl, we have:
  \begin{align*}
    \Div \, \Curl \, \mathbf{F} & = \frac{\partial}{\partial x} \left( \frac{\partial R}{\partial y} - \frac{\partial Q}{\partial z} \right) + \frac{\partial}{\partial y} \left( \frac{\partial P}{\partial z} - \frac{\partial R}{\partial x} \right) + \frac{\partial}{\partial z} \left( \frac{\partial Q}{\partial x} - \frac{\partial P}{\partial y} \right) \\
...
The Green's Theorem considered the vector field in the direction tangent to the curve. We may also consider the vector field in the direction normal to the curve. Then we have:
\begin{equation}
  \oint_C \mathbf{F} \cdot \mathbf{n} \, ds = \oint_C -Q \, dx + P \, dy = \iint_D \left( \frac{\partial P}{\partial x} + \frac{\partial Q}{\partial y} \right) \, dA = \iint_D \Div \, \mathbf{F} \, dA.
\end{equation}
where $\mathbf{n}$ is the unit normal vector to the curve $C$ and $D$ is the region enclosed by the curve $C$. Sometimes this is called the \emph{flux} of the vector field $\mathbf{F}$ across the curve $C$.

\begin{example}
  Find the flux of the vector field $\mathbf{F}(x,y) = \langle x-y, x \rangle$ across the circle $x^2 + y^2 = 1$ on the $xy$-plane.
\end{example}
\begin{solution}
  We have:
  \[
    \Div \, \mathbf{F} = \frac{\partial}{\partial x} (x - y) + \frac{\partial}{\partial y} (x) = 1 + 0 = 1.
  \]
  Let $D$ be the region enclosed by the circle $x^2 + y^2 = 1$. Then the flux of the vector field $\mathbf{F}$ across the circle is given by:
  \begin{align*}
    \oint_C \mathbf{F} \cdot \mathbf{n} \, ds & = \iint_D \Div \, \mathbf{F} \, dA = \iint_D 1 \, dA = \text{Area}(D) = \pi (1)^2 = \pi.
  \end{align*}
\end{solution}
...
The surface integral of $f$ over a surface $S$ parameterised by $\mathbf{r}(u,v) = \langle x(u,v), y(u,v), z(u,v) \rangle$ for $(u,v) \in D$ is defined as:
\begin{equation}
  \iint_S f(x,y,z) \, d\sigma = \lim_{m, n \to \infty} \sum_{i=1}^m \sum_{j=1}^n f(x(u_{ij}, v_{ij}), y(u_{ij}, v_{ij}), z(u_{ij}, v_{ij})) \Delta S_{ij},
\end{equation}
where $\Delta S_{ij}$ is the area of the small patch on the surface $S$ corresponding to the subrectangle in $D$ containing the point $(u_{ij}, v_{ij})$. We have:
\[
  \Delta S_{ij} \approx \left\| \mathbf{r}_u \times \mathbf{r}_v \right\| \Delta A.
\]
Thus, the surface integral of a scalar function $f$ over a surface $S$ can be computed as:
\begin{equation}
  \iint_S f(x,y,z) \, d\sigma = \iint_D f(\mathbf{r}(u,v)) \left\| \mathbf{r}_u \times \mathbf{r}_v \right\| \, dA.
\end{equation}
where $D$ is the projection of the surface $S$ onto the $uv$-plane. You may compare this with the formula for line integrals of scalar functions, Equation~\ref{eq:line_integral_scalar}.

\begin{example}
  Evaluate the surface integral $\iint_S x^2 \, d\sigma$ where $S$ is the unit sphere $x^2 + y^2 + z^2 = 1$.
\end{example}
\begin{solution}
  We can parameterise the unit sphere using spherical coordinates:
  \[
    \mathbf{r}(\theta, \phi) = \langle \sin \phi \cos \theta, \sin \phi \sin \theta, \cos \phi \rangle,
  \]
  where $0 \leq \theta \leq 2\pi$ and $0 \leq \phi \leq \pi$. Then we have:
  \begin{align*}
  ...
  \end{align*}
  Thus, we compute the cross product:
  \[
    \left\| \mathbf{r}_\theta \times \mathbf{r}_\phi \right\| = \sin \phi.
  \]
  Then the surface integral becomes:
  \begin{align*}
    \iint_S x^2 \, d\sigma & = \int_0^{2\pi} \int_0^{\pi} (\sin \phi \cos \theta)^2 \cdot \sin \phi \, d\phi \, d\theta \\
                      & = \int_0^{2\pi} \int_0^{\pi} \sin^3 \phi \cos^2 \theta \, d\phi \, d\theta                 \\
                      & = \int_0^{2\pi} \cos^2 \theta \, d\theta \cdot \int_0^{\pi} \sin^3 \phi \, d\phi           \\
                      & = \pi \cdot \frac{4}{3} = \frac{4\pi}{3}.
  \end{align*}
\end{solution}

If the surface $S$ is given by the graph of a function $z = g(x,y)$ over a region $R_{xy}$ in the $xy$-plane, then we consider $f(x,y,z) = f(x,y,g(x,y))$ defined on $R_{xy}$. The surface integral of $f$ over the surface $S$ is given by:
\begin{equation}
  \iint_S f(x,y,z) \, d\sigma = \iint_{R_{xy}} f(x,y,g(x,y)) \sqrt{\left( \frac{\partial g}{\partial x} \right)^2 + \left( \frac{\partial g}{\partial y} \right)^2 + 1} \, dA.
\end{equation}
If the surface $S$ is given by the graph of a function $y = h(x,z)$ over a region $R_{xz}$ in the $xz$-plane, then we consider $f(x,y,z) = f(x,h(x,z),z)$ defined on $R_{xz}$. The surface integral of $f$ over the surface $S$ is given by:
\begin{equation}
  \iint_S f(x,y,z) \, d\sigma = \iint_{R_{xz}} f(x,h(x,z),z) \sqrt{\left( \frac{\partial h}{\partial x} \right)^2 + 1 + \left( \frac{\partial h}{\partial z} \right)^2} \, dA.
\end{equation}
If the surface $S$ is given by the graph of a function $x = k(y,z)$ over a region $R_{yz}$ in the $yz$-plane, then we consider $f(x,y,z) = f(k(y,z),y,z)$ defined on $R_{yz}$. The surface integral of $f$ over the surface $S$ is given by:
\begin{equation}
  \iint_S f(x,y,z) \, d\sigma = \iint_{R_{yz}} f(k(y,z),y,z) \sqrt{1 + \left( \frac{\partial k}{\partial y} \right)^2 + \left( \frac{\partial k}{\partial z} \right)^2} \, dA.
\end{equation}

\subsection{Surface Integrals of vector fields}

\begin{definition}[Orientation of surfaces]
  An \emph{orientation} of a surface $S$ is a choice of a continuous family of normal vector field \(\mathbf{n}\) of length 1 on the surface. A surface S is said to be orientable if such a normal vector field exists. A surface S is said to be oriented if a choice of an orientation is specified.
\end{definition}
For example, a sphere is orientable since we can choose the outward normal vector at each point on the sphere. A Möbius strip is not orientable since if we try to choose a normal vector field continuously, we will end up with a normal vector pointing in the opposite direction after going around the strip once.

For a closed orientable surface, the outward normal vector is usually chosen as the orientation. It is called the \emph{positive orientation} of the surface.

If $\mathbf{F}(x,y,z) = \langle P(x,y,z), Q(x,y,z), R(x,y,z) \rangle$ is a vector field in three dimensions and $S$ is an oriented surface with unit normal vector field $\mathbf{n}$, then the surface integral of the vector field $\mathbf{F}$ over the surface $S$ is defined as:
\begin{equation}
  \iint_S \mathbf{F} \cdot \mathbf{n} \, d\sigma = \iint_S \mathbf{F} \cdot d\mathbf{S} = \iint_S \mathbf{F} \cdot (\mathbf{r}_u \times \mathbf{r}_v) \, dA
\end{equation}
where $\mathbf{r}(u,v) = \langle x(u,v), y(u,v), z(u,v) \rangle$ is a parameterisation of the surface $S$ for $(u,v) \in D$.

\subsection{Stokes' Theorem}

\begin{theorem}
  Let $S$ be an oriented, piecewise smooth surface bounded by a simple, closed, piecewise smooth curve $\partial S$ with positive orientation. If $\mathbf{F}(x,y,z) = \langle P(x,y,z), Q(x,y,z), R(x,y,z) \rangle$ is a vector field whose components have continuous first partial derivatives on an open region that contains $S$, then we have:
  \begin{equation}
    \oint_{\partial S} \mathbf{F} \cdot \mathbf{T} \, ds = \iint_S \Curl \, \mathbf{F} \cdot \mathbf{n} \, d\sigma.
  \end{equation}
\end{theorem}

Consider the differential 1-form $\omega = P \, dx + Q \, dy + R \, dz$ on the surface $S$, and its exterior derivative is given by:
\begin{align*}
  d\omega & = dP \wedge dx + P \, d(dx) + dQ \wedge dy + Q \, d(dy) + dR \wedge dz + R \, d(dz)                                                                                                                                                                                                          \\
          ...
          & = (Q_x - P_y) \, dx \wedge dy + (R_y - Q_z) \, dy \wedge dz + (P_z - R_x) \, dz \wedge dx.
\end{align*}
Thus, by the generalised Stokes' Theorem, we have:
\[
  \oint_{\partial S} P \, dx + Q \, dy + R \, dz = \iint_S \left[ (Q_x - P_y) \, dx \wedge dy + (R_y - Q_z) \, dy \wedge dz + (P_z - R_x) \, dz \wedge dx \right].
\]

\subsection{The Divergence Theorem}

\begin{theorem}
  Let $E$ be a simple solid region bounded by a closed, piecewise smooth surface $\partial E$ with positive orientation. If $\mathbf{F}(x,y,z) = \langle P(x,y,z), Q(x,y,z), R(x,y,z) \rangle$ is a vector field whose components have continuous first partial derivatives on an open region that contains $E$, then we have:
  \begin{equation}
    \oiint\nolimits_{\partial E} \mathbf{F} \cdot d\mathbf{S} = \iiint_E \Div \, \mathbf{F} \, dV.
  \end{equation}
\end{theorem}

\begin{example}
  Evaluate the surface integral $\iint_S \mathbf{F} \cdot \mathbf{n} \, d\sigma$ where $\mathbf{F}(x,y,z) = \langle z+x, y, 1 \rangle$ and $S$ is the surface of an upper hemisphere $z = \sqrt{1-x^2-y^2}$, without the bottom disk, and $\mathbf{n}$ is the outward unit normal vector.
\end{example}
\begin{solution}
  Let $D$ be the bottom disk of the hemisphere $S$. Then $S \cup D$ is a closed surface bounding the solid hemisphere $E$. By the Divergence Theorem, we have:
  \[
    \oiint\nolimits_{S \cup D} \mathbf{F} \cdot \mathbf{n} \, d\sigma = \iiint_E \Div \, \mathbf{F} \, dV.
  \]
  where $E$ is the solid hemisphere bounded by the surface $S \cup D$. Then we have:
  \[
    \oiint\nolimits_{S \cup D} \mathbf{F} \cdot \mathbf{n} \, d\sigma = \iiint_E 2 \, dV = 2 \cdot \frac{2}{3} \pi (1)^3 = \frac{4\pi}{3}.
  \]
  Next, we have:
  \begin{align*}
    \iint_S \mathbf{F} \cdot \mathbf{n} \, d\sigma & = \frac{4\pi}{3} - \iint_D \mathbf{F} \cdot \mathbf{n} \, d\sigma = \frac{4\pi}{3} - \iint_D \mathbf{F} \cdot (-\mathbf{k}) \, d\sigma = \frac{4\pi}{3} + \iint_D 1 \, d\sigma = \frac{7\pi}{3}.
  \end{align*}
\end{solution}

We may conclude this chapter with the following equations. There are four important vector calculus theorems of vector fields that relate integrals over different dimensions:
\begin{align*}
  \text{Fundamental Theorem of Line Integrals:} & \quad \int_C \nabla f \cdot d\mathbf{r} = f(\mathbf{r}(b)) - f(\mathbf{r}(a)),                         \\
  \text{Green's Theorem (Tangent form):}        & \quad \oint_C \mathbf{F} \cdot \mathbf{T} \, ds = \iint_D \Curl \, \mathbf{F} \cdot \mathbf{k} \, dA,     \\
  \text{Stokes' Theorem:}                       & \quad \oint_{\partial S} \mathbf{F} \cdot \mathbf{T} \, ds = \iint_S \Curl \, \mathbf{F} \cdot \mathbf{n} \, d\sigma, \\
  \text{Green's Theorem (Normal form):}         & \quad \oint_C \mathbf{F} \cdot \mathbf{n} \, ds = \iint_D \Div \, \mathbf{F} \, dA,                       \\
  \text{Divergence Theorem:}                    & \quad \oiint\nolimits_{\partial E} \mathbf{F} \cdot d\mathbf{S} = \iiint_E \Div \, \mathbf{F} \, dV.
\end{align*}
The tangent form of Green's Theorem is also called the curl form or circulation form of Green's Theorem. The normal form of Green's Theorem is also called the divergence form or flux form of Green's Theorem. Note that the flux is positive if the vector field points outward from the region.

We also have two important equations for line integral and surface integral of scalar functions:
\begin{align*}
  \text{Line integral of scalar functions:}    & \quad \int_C f(x,y,z) \, ds = \int_a^b f(\mathbf{r}(t)) \| \mathbf{r}'(t) \| \, dt,                                                                                    \\
  \text{Surface integral of scalar functions:} & \quad \iint_S f(x,y,z) \, d\sigma = \iint_D f(\mathbf{r}(u,v)) \left\| \mathbf{r}_u \times \mathbf{r}_v \right\| \, dA.
\end{align*}
